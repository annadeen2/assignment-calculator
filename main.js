...
function item_input_listener(e) {
	...
	qty = parseInt(qty);
	const grades = row.querySelectorAll(".regular, .midgrade, .premium, .diesel");
	row.classList.add("active");
	grades.forEach(function (grade) {
		/**
		 * capture the price for this item, at this shop, from the data attribute and turn it into a float
		 **/
		const price = parseFloat(grade.dataset.price);
		/**
		 ...
		 */
		grade.querySelector("span").innerHTML = round_number(total);
	});
...
function calculate_totals() {
	...
	item_rows.forEach(function (row) {
		...
		//for each shop
		grades.forEach(function (grade) {
			const price = parseFloat(grade.dataset.price); //get this shop's price for this row's item as a float
			const total = price * qty; //calculate the total by multiplying price X quantity
			// use the classes to figure out what shop this is
			if (grade.classList.contains("regular")) { //if it's amazon, increase amazon's total
				regular = regular + total; // come back
			}
			if (grade.classList.contains("midgrade")) { //if it's peapod, increase peapod's total
				midgrade = midgrade + total; // come back
			}
			if (grade.classList.contains("premium")) { //if it's freshdirect, increase freshdirect's total
				premium = premium + total; //come back
			}
			if (grade.classList.contains("diesel")) { //if it's freshdirect, increase freshdirect's total
				diesel = diesel + total; //come back
			}
		});
	});
	...
	total_row.querySelector('.regular span').innerHTML = round_number(regular); // set the total row's amazon container content to be the round number for amazon's total
	total_row.querySelector('.midgrade span').innerHTML = round_number(midgrade); // set the total row's peapod container content to be the round number for peapod's total
	total_row.querySelector('.premium span').innerHTML = round_number(premium);
	total_row.querySelector('.diesel span').innerHTML = round_number(diesel); // set the total row's freshdirect container content to be the round number for freshdirect's total
	/**
	 * Now let's use conditionals to determine which shop cost least
	 */
	if (regular < midgrade && regular < premium && regular < diesel) { // if amazon's cost is less than both peapod and freshdirect, amazon is cheapest
		cheapest = 'regular';
	}
	if (midgrade < regular && midgrade < premium && midgrade < diesel) { // if peapod's cost is less than both amazon and freshdirect, peapod is cheapest
		cheapest = 'midgrade';
	}
	if (premium < regular && premium < midgrade && premium < diesel) { // if freshdirect's cost is less than both peapod and amazon, freshdirect is cheapest
		cheapest = 'premium';
	}
	if (diesel < regular && diesel < midgrade && diesel < premium) { // if freshdirect's cost is less than both peapod and amazon, freshdirect is cheapest
		cheapest = 'diesel';
	}
